...
from seg.ui import *
from seg.general import *
from seg.files import *

from seg.audio_segment import *
from seg.features import *
from seg.dataset import *
from seg.evaluation import *
...
def main(argv):
    ...
    # eval函数将字符串当成有效Python表达式来求值  建一个TUTAcousticScenes_2016_DevelopmentSet类变量
    dataset = eval(params['general']['development_dataset'])(data_path=params['path']['data'])

    # Fetch data over internet and setup the data
    # ==================================================

    # initialize should be false when you have download the database
    if params['flow']['initialize']:
        dataset.fetch()

    # do audio segment
    # ==================================================
    if params['flow']['segment']:
        section_header('segment')

        # Collect files in train sets

        do_segment(dataset=dataset,segment_txt=params['path']['segment_txt'],
                   segment_wav=params['path']['segment_wav'],
...
def process_parameters(params):
    ...
    # segment
    params['path']['segment_txt_'] = params['path']['segment_txt']
    params['path']['segment_txt'] = os.path.join(params['path']['base'],
                                                 params['path']['segment_txt_'])

    params['path']['segment_wav_'] = params['path']['segment_wav']
    params['path']['segment_wav'] = os.path.join(params['path']['base'],
                                                 params['path']['segment_wav_'])
...
def make_folders(params, parameter_filename='parameters.yaml'):
    ...
    check_path(params['path']['segment_wav'])
    check_path(params['path']['segment_txt'])
        ...
        if not os.path.isfile(current_model_file) or overwrite:
            ...
            for label in data:
                progress(title_text='Train models',
                         ...
                         note=label)
                if classifier_method == 'gmm':
                    # print classifier_params['n_components']
                    # classifier_params['n_components'] = len(data)+1
...
def do_system_evaluation(dataset, result_path, dataset_evaluation_mode='folds'):
    ...
    for fold in dataset.folds(mode=dataset_evaluation_mode):
        ...
        for result in results:
            ...
            # print result[1]
            str1 = result[1][0:len(result[1]) - 1]
            y_pred.append(str1)
